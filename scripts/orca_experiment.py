...
def det_scheduler() -> None:
    "Tell Orca to guess at what the ideal scheduler should be."

    cmdargs = ["bazel-bin/orca_client", "detsched"]
    subprocess.run(cmdargs, check=True)
...
def run_sched_exp(sched_type: str, proportion_long_jobs: Decimal) -> List[List[str]]:
    ...
    if sched_type == "orca":
        # Let Orca pick the scheduler to start (also flushes the metrics)
        det_scheduler()

        # Make one profiling run
        ...
        # Now let Orca pick the scheduler again. The choice should be fine tuned to the workload.
        det_scheduler()
    else:
        set_scheduler(
            sched_type=sched_type, preemption_interval_us=preemption_interval_us
        )
